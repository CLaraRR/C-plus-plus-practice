	...
	for (int i = 0; i <= n; i++){
		for (int j = 0; j <= capacity; j++){
			if (i == 0 || j == 0){ //边界
				...
				dp[i][j] = max(dp[i - 1][j], dp[i - 1][j - w[i]] + v[i]);	//如果“不装”，则容量为j时前i件物品的最大价值等于容量为j时前i-1件物品的最大价值
				//如果“装”，则容量为j时前i件物品的最大价值等于当减去当前物品的重量得到的剩余容量为j-w[i]时前i-1件物品的最大价值，然后再加上当前物品的价值所得到的价值
